...
router.get(
  '/complete/:id',
  auth(USER_ROLES.USER),
  BookingController.completeBookingToDB
);
router.patch(
  '/reschedule/:id',
  auth(USER_ROLES.USER),
  fileUploadHandler(),
  BookingController.rescheduleBookingToDB
);
router.get(
  '/check-availability/:id',
  auth(USER_ROLES.USER),
  BookingController.checkAvailabilityBookingFromDB
);
router.get(
  '/transactions-history',
  auth(USER_ROLES.USER, USER_ROLES.ARTIST),
  BookingController.transactionsHistoryFromDB
);
router.get(
  '/booking-summary',
  auth(USER_ROLES.ARTIST),
  BookingController.bookingSummaryFromDB
);
router.get(
  '/lesson-booking',
  auth(USER_ROLES.ARTIST),
  BookingController.lessonBookingSummary
);
router.post(
  '/send-link/:id',
  auth(USER_ROLES.ARTIST),
  fileUploadHandler(),
  BookingController.sendLinkToUser
);
router.patch(
  '/respond/:id',
  auth(USER_ROLES.USER, USER_ROLES.ARTIST),

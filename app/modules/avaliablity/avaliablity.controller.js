...
const setAvailability = (0, catchAsync_1.default)(async (req, res) => {
    const artistId = req.user.id; // must be ARTIST role
    const { date, slots } = req.body;
    const result = await avaliablity_service_1.AvailabilityService.setAvailability(artistId, date, slots);
...
const getAvailability = (0, catchAsync_1.default)(async (req, res) => {
    const { artistId, date } = req.query;
    // 1. Get base availability slots (from Availability collection)
    const baseSlots = await avaliablity_service_1.AvailabilityService.getAvailability(artistId, date);
    if (!baseSlots) {
    ...
    const slotsWithStatus = await Promise.all(baseSlots.slots.map(async (slot) => {

...
const artistProfileFromDB = async (payload) => {
    const isExistArtist = await user_model_1.User.findById(payload)
        .populate({
        path: "lesson",
    ...
    const bookmarkId = await bookmark_model_1.Bookmark.find({ artist: payload }).distinct("artist");
    const reviews = await review_model_1.Review.find({ artist: payload })
        .populate({
        path: "user",
        select: "name profile"
...
const popularArtistFromDB = async () => {
    const artists = await lesson_model_1.Lesson.find({ rating: { $gt: 0 } })
        .populate({
        path: "user",
        select: "name profile"
    }).select("gallery  user rating totalRating lessonTitle duration").sort({ rating: -1 });
    // get all artist id from bookmark;
    const bookmarkId = await bookmark_model_1.Bookmark.find({}).distinct("artist");
    const bookmarkIdStrings = bookmarkId.map(id => id.toString());
...
const artistByCategoryFromDB = async (category) => {
    const artists = await lesson_model_1.Lesson.find({ genre: category })
        .populate({
        path: "user",
        select: "name profile"
...
const availableArtistFromDB = async () => {
    // Get IDs of artist who have made a booking
    const bookedArtistIds = await booking_model_1.Booking.distinct("artist");
    console.log(bookedArtistIds);
    // Find users who are not in the list of bookedArtistIds
    const availableArtists = await user_model_1.User.find({ _id: { $nin: bookedArtistIds }, role: "ARTIST" })
        .populate({
        path: "lesson",

...
const createBooking = async (payload) => {
    ...
    const slot = await avaliablity_model_1.Availability.findOne({
    ...
    const createOrder = {
        ...payload,
        bookingId: await (0, generateBookingId_1.default)(),
    };
    const booking = await booking_model_1.Booking.create(createOrder);
    if (!booking) {
        throw new ApiError_1.default(http_status_codes_1.StatusCodes.BAD_REQUEST, 'Failed to create booking');
    }
    // Mark the slot as booked
    await avaliablity_model_1.Availability.updateOne({
        artist: payload.artist,
        date: payload.booking_date,
        'slots.time': payload.booking_time,
...
const myBookingFromDB = async (id, queries) => {
    const query = {
        users: { $in: [id] },
    };
    if (queries === 'Complete') {
...
const completeBookingToDB = async (id) => {
    const isExistBooking = await booking_model_1.Booking.findByIdAndUpdate({ _id: id }, { status: 'Complete' }, { new: true });
    if (!isExistBooking) {
...
// reschedule booking
const rescheduleBookingToDB = async (id, payload) => {
    const isExistBooking = await booking_model_1.Booking.findById(id);
    if (!isExistBooking) {
        throw new ApiError_1.default(http_status_codes_1.StatusCodes.NOT_FOUND, 'There is No Booking Found');
    }
    const updatedData = {
    ...
    await (0, notificationsHelper_1.sendNotifications)(data);
    const result = await booking_model_1.Booking.findByIdAndUpdate({ _id: id }, updatedData, {
        new: true,
    });
    return result;
};
// check booking availability
const checkAvailabilityBookingFromDB = async (id, date) => {
    // Convert date to yyyy-mm-dd format
    const today = new Date().toISOString().split('T')[0];
    // Get all booked dates for the artist from today onwards
    const bookingList = await booking_model_1.Booking.find({
        artist: id,
    ...
    const getBookingTimes = await booking_model_1.Booking.find({
        artist: id,
...
// transaction history
const transactionsHistoryFromDB = async (user) => {
    const role = user?.role;
    const query = role === 'USER' ? { user: user?.id } : { artist: user?.id };
...
const respondBookingToDB = async (id, status) => {
    ...
    // ✅ update booking
    const result = await booking_model_1.Booking.findByIdAndUpdate(id, { status: normalizedStatus }, { new: true });
    if (!result) {
        throw new ApiError_1.default(http_status_codes_1.StatusCodes.BAD_REQUEST, 'Failed to update booking.');
    }
    // ✅ refund if rejected
    if (normalizedStatus === 'Rejected') {
        const paymentIntent = await stripe.paymentIntents.retrieve(result?.transactionId);
...
// booking details
const bookingDetailsFromDB = async (id) => {
...
// check booking availability
const bookingSummaryFromDB = async (id) => {
    // Convert date to yyyy-mm-dd format
    const today = new Date().toISOString().split('T')[0];
    // Get all booked user
    const bookingList = await booking_model_1.Booking.find({
    ...
    // my balance
    const totalIncome = await booking_model_1.Booking.aggregate([
    ...
    const endOfYear = new Date(newDate.getFullYear(), 11, 31)
        .toISOString()
        .split('T')[0];
    const yearlyBooking = await booking_model_1.Booking.find({
        artist: id,
...
const lessonBookingFromDB = async (id, status, date) => {
    // today in yyyy-mm-dd
    const today = new Date().toISOString().split('T')[0];
    // Base query
    let query = {
...
const sendLinkToUser = async (id, bookingLink) => {
    const booking = await booking_model_1.Booking.findById(id).populate('user artist');
    const emailData = {
    ...
    await (0, notificationsHelper_1.sendNotifications)(data);
    await emailHelper_1.emailHelper.sendLink(emailData);
};

...
// ---------------- CREATE PAYMENT INTENT ----------------
const createPaymentIntentToStripe = async (user, payload) => {
    ...
    if (!musicianId) {
        throw new ApiError_1.default(http_status_codes_1.StatusCodes.BAD_REQUEST, 'Musician ID is required');
    }
    // Find the musician to get their Stripe Account ID
    const musician = await user_model_1.User.findById(musicianId);
    if (!musician || !musician.accountInformation?.stripeAccountId) {
        throw new ApiError_1.default(http_status_codes_1.StatusCodes.NOT_FOUND, 'Musician has no Stripe account configured');
    }
    ...
    if (!paymentIntent.client_secret) {
        throw new ApiError_1.default(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, 'Failed to create Payment Intent');
    }
    const date = new Date();
    const txid = 'Txid' +
        date.getTime() +
...
const createAccountToStripe = async (user) => {
    // user check
    ...
    // already has account?
    if (await user_model_1.User.isAccountCreated(user.id)) {
...
const verifyStripeAccountStatus = async (userId) => {
    const user = await user_model_1.User.findById(userId);
    if (!user || !user.accountInformation?.stripeAccountId) {
        throw new ApiError_1.default(http_status_codes_1.StatusCodes.BAD_REQUEST, 'Stripe account not found');
    }
    const account = await stripe.accounts.retrieve(user.accountInformation.stripeAccountId);
...
// transfer and payout credit
const transferAndPayoutToArtist = async (id) => {
    ...
    const isExistArtist = await user_model_1.User.isAccountCreated(artist);
    if (!isExistArtist) {
    ...
    if (transfer.id && payouts.id) {
        await booking_model_1.Booking.findByIdAndUpdate({ _id: id }, { status: 'Complete' }, { new: true });
    }
    return;
};
const handleCheckoutCompleted = async (session) => {
    try {
        ...
        console.log(`✅ Payment successful for ${customerEmail}, Amount: $${amount}`);
        ...
        if (user) {
            await user_model_1.User.findByIdAndUpdate(user._id, {
                $set: {
                    ...
                    'accountInformation.chargesEnabled': account.charges_enabled,
                    'accountInformation.payoutsEnabled': account.payouts_enabled,
                    'accountInformation.status': account.charges_enabled && account.payouts_enabled,
                },
            });
        }
    }
...
const handlePayoutPaid = async (payout) => {
    console.log(`✅ Payout completed: ${payout.id} - Amount: ${payout.amount / 100} ${payout.currency}`);
};

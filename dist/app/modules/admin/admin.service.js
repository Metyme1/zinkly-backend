...
const createSuperAdminToDB = (payload) => __awaiter(void 0, void 0, void 0, function* () {
    if (payload.role !== "SUPER_ADMIN") {
        throw new ApiError_1.default(http_status_codes_1.StatusCodes.BAD_REQUEST, "This Api only for Super ADMIN");
    }
    const isExistEmail = yield user_model_1.User.findOne({ email: payload.email });
    if (isExistEmail) {
...
const usersFromDB = (payload) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const result = yield user_model_1.User.find(whereConditions).select("name email contact location gender profile").skip(skip).limit(size);
...
const artistFromDB = (payload) => __awaiter(void 0, void 0, void 0, function* () {
    const { search, page, limit } = payload;
    const anyConditions = [];
    anyConditions.push({
        role: "ARTIST"
...
const transactionsFromDB = (payload) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const count = yield booking_model_1.Booking.countDocuments(whereConditions);
    const data = {
        data: result,
        meta: {
            page: pages,
...
const bookingSummaryFromDB = () => __awaiter(void 0, void 0, void 0, function* () {
    // total user
    const users = yield user_model_1.User.countDocuments({ role: "USER" });
    // total artist
    const artist = yield user_model_1.User.countDocuments({ role: "ARTIST" });
    // balance
    const income = yield booking_model_1.Booking.aggregate([
        {
            $group: {
                _id: null,
...
const earningStatisticFromDB = () => __awaiter(void 0, void 0, void 0, function* () {
    // month with 0 income
    const months = [
    ...
    const monthlyEarnings = yield booking_model_1.Booking.aggregate([
...
const userStatisticFromDB = () => __awaiter(void 0, void 0, void 0, function* () {
    const months = [
    ...
    const monthlyUser = yield user_model_1.User.aggregate([
        { $match: { role: "USER", createdAt: { $gte: startDate, $lt: endDate } } },
        { $group: { _id: { month: { $month: "$createdAt" } }, count: { $sum: 1 } } }
    ]);
    // Aggregate artists by month
    const monthlyArtist = yield user_model_1.User.aggregate([
